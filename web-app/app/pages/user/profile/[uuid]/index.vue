...
<template>
  <div class="flex flex-col gap-4 w-full px-2">
    ...
    <div class="flex flex-col gap-3">
      <!-- Roles Section -->
      <div class="bg-gray-900/40 rounded-2xl overflow-hidden border border-white/5 transition-all duration-300"
        :class="{ 'bg-gray-800/60 border-indigo-500/20': activeTab === '0' }">
        <button @click="toggleTab('0')" class="w-full flex items-center justify-between p-4 group">
          <div class="flex items-center gap-3">
            ...
            <span class="text-md font-semibold text-gray-200 group-hover:text-white transition-colors">Roles de
              Usuario</span>
          </div>
          <Icon name="material-symbols:keyboard-arrow-down-rounded" size="24"
            class="text-gray-500 transition-transform duration-300" :class="{ 'rotate-180': activeTab === '0' }" />
        </button>
        <div v-show="activeTab === '0'" class="px-4 pb-4 transition-all duration-300">
      ...
      <div class="bg-gray-900/40 rounded-2xl overflow-hidden border border-white/5 transition-all duration-300"
        ...
        <button @click="toggleTab('1')" class="w-full flex items-center justify-between p-4 group">
          <div class="flex items-center gap-3">
            <Icon name="material-symbols:account-balance-wallet-outline-rounded" size="22"
              class="text-indigo-400 group-hover:scale-110 transition-transform duration-300" />
            <span class="text-md font-semibold text-gray-200 group-hover:text-white transition-colors">Billetera
              Digital</span>
          </div>
          <Icon name="material-symbols:keyboard-arrow-down-rounded" size="24"
            class="text-gray-500 transition-transform duration-300" :class="{ 'rotate-180': activeTab === '1' }" />
        </button>
      ...
      <div class="bg-gray-900/40 rounded-2xl overflow-hidden border border-white/5 transition-all duration-300"
        ...
        <button @click="toggleTab('2')" class="w-full flex items-center justify-between p-4 group">
          <div class="flex items-center gap-3">
            <Icon name="material-symbols:trophy-outline-rounded" size="22"
              class="text-indigo-400 group-hover:scale-110 transition-transform duration-300" />
            <span class="text-md font-semibold text-gray-200 group-hover:text-white transition-colors">Logros y
              Hazañas</span>
          </div>
          <Icon name="material-symbols:keyboard-arrow-down-rounded" size="24"
            class="text-gray-500 transition-transform duration-300" :class="{ 'rotate-180': activeTab === '2' }" />
        </button>
