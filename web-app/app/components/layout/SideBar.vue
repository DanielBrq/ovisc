...
const emit = defineEmits(["update:sidenavOpen", "update:active"]);
...
<template>
  <nav>
    <aside
      class="fixed top-0 left-0 h-full bg-ovis-neutral-900 border-r border-ovis-neutral-800 text-ovis-neutral-50 flex flex-col transition-all duration-300 ease-in-out z-50 overflow-hidden"
      ...
      <div class="h-16 flex items-center px-4 shrink-0 overflow-hidden">
        ...
        >
          <span
            ...
            class="font-bold text-xl ovis-gradient-w truncate"
            >OVIS</span
          >
          <button
            @click="toggleSidebar"
            class="p-2 rounded-lg hover:bg-ovis-neutral-800 transition-colors text-ovis-neutral-400 hover:text-ovis-neutral-50 cursor-pointer"
          >
            <Icon
              size="24"
              :name="
          ...
          <li v-for="item in menuItems" :key="item.key">
            ...
            <template v-if="item.subItems">
              <layout-side-bar-button
                @toggle-submenu="toggleSubmenu(item.key)"
                :sidenavOpen="sidenavOpen"
                :active="item.key"
                :arrow="true"
                :submenuIsOpen="openSubmenus[item.key]"
                :isActive="props.active === item.key"
                :icon-name="item.icon"
                :to="item.to"
              >
                {{ item.label }}
              ...
              <div
                ...
                :class="[
                  openSubmenus[item.key] && props.sidenavOpen
                    ? 'max-h-64 opacity-100 mt-1'
                    : 'max-h-0 opacity-0',
                ]"
              >
                <div
                  class="flex flex-col gap-1 pl-4 border-l border-ovis-neutral-800 ml-6"
                >
